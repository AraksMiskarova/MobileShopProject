...
exports.refreshToken = (req, res) => {
    ...
    jwt.verify(refreshToken, keys.secretOrKey,  async (err, decoded) => {
        ...
        await Session.findOne({_id: decoded.sid }).then( async(ses)=>{
            ...
            if(!ses) return  res.status(401).json({status:false, message:'Not found session'})
            
            // Create JWT Payload
            const getCustomer = await Customer.findOne({_id: decoded.uid})
            if(!getCustomer)return res.status(401).json({ status:false, message: "Customer not found" })
